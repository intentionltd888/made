...
{id:'fully',body:'fully_body.png',face:'fully_face.png',mark:'fully_glyph.png',cat:'apps',href:'/fully',os:'macOS',wm:'fully_logotype_black.png',alt:'fully',line:"Paste the link. Get it fully.",zh:'網頁上的圖，存成原尺寸',tags:'不是縮圖、不上傳',side:[196,72,8],glyph:[250,247,242]}
];
var BODY=14,GLYPH=6;
function icon(it){
...
(function(){var v=$('.mark video');if(!v)return;v.muted=true;v.defaultMuted=true;v.playsInline=true;v.setAttribute('muted','');v.setAttribute('playsinline','');v.setAttribute('webkit-playsinline','');
function go(){if(v.paused){var pr=v.play();if(pr&&pr.catch)pr.catch(function(){})}}
var m=v.parentNode;v.style.opacity='0';
function live(){if(v.currentTime>0&&!v.paused&&v.readyState>2){v.style.opacity='1';m.classList.add('on')}}
v.addEventListener('playing',live);v.addEventListener('timeupdate',live);
go();['touchend','click','scroll'].forEach(function(ev){document.addEventListener(ev,go,{passive:true})})})();
...
var dmg=host.getAttribute('dmg-url')||DEF.dmg,repo=host.getAttribute('repo-url')||DEF.repo,win=host.getAttribute('win-url')||DEF.win;
var A=function(n){return MAP[n]||base+n};
var fill=function(s){return s.replace(/@@A:([^@]+)@@/g,function(_,n){return A(n)}).split('@@DMG@@').join(dmg).split('@@REPO@@').join(repo).split('@@WIN@@').join(win)};
